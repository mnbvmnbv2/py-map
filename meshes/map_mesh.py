...
class MapMesh(BaseMesh):
    ...
    def get_vertex_data(self):
        ...
        vertex_data[::18] = np.tile(
            np.arange(self.width) / (self.width / 2) - 1, self.height
        )
        # y
        vertex_data[1::18] = np.repeat(
            (np.arange(self.height) + 1) / (self.height / 2) - 1, self.width
        )[::-1]
        # z
        ...
        vertex_data[4::18] = np.repeat(
            np.arange(self.height) / (self.height / 2) - 1, self.width
        )[::-1]
        vertex_data[5::18] = height_map[1:, :-1].flatten()
        # upper right
        vertex_data[6::18] = np.tile(
            (np.arange(self.width) + 1) / (self.width / 2) - 1, self.height
        )
